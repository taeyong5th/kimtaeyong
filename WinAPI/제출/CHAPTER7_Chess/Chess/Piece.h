...
public:
	virtual std::vector<std::pair<BOARD_POSITION_X, BOARD_POSITION_Y>> getMovablePositions(Board* board) = 0; // piece가 이동할 수 있는 모든좌표 반환
	
	bool move(Board* board, BOARD_POSITION_X x, BOARD_POSITION_Y y); // board의 x, y 위치로 피스를 이동	
