...
void BitmapManager::draw(HDC hdc, LPCWSTR fileName, int x, int y, float px, float py, float sx, float sy, float ex, float ey)
{
	std::map<LPCWSTR, Bitmap*>::iterator it = m_BitmapList.find(fileName);

	if (it != m_BitmapList.end())
	{	
		(it->second)->draw(hdc, x, y, px, py, sx, sy, ex, ey);
	}
}
