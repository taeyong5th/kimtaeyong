...
public:
	...
	void draw(HDC hdc, LPCWSTR fileName, int x, int y, float px = 1.0f, float py = 1.0f, float sx = 0.0f, float sy = 0.0f, float ex = 1.0f, float ey = 1.0f); // fileName의 비트맵 파일을 그린다. x, y : 그려질 좌상위치, px, py : 원본 비트맵 너비의 px배,  높이의 py 배로 그림
};