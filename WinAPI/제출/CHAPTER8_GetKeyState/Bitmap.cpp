...
void Bitmap::draw(HDC hdc, int x, int y, float px, float py, float sx, float sy, float ex, float ey)
{
	PAINTSTRUCT ps;
	HBITMAP oldBitmap;
	...
	TransparentBlt(hdc, x, y, m_iWidth * (ex - sx) * px , m_iHeight * (ey - sy) * py , m_MemDC, m_iWidth * sx, m_iHeight * sy, m_iWidth * (ex - sx), m_iHeight * (ey - sy), RGB(255, 0, 255));
