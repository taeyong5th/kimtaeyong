...
float					g_fRotX = 0.0f;
float					g_fRotY = 0.0f;
D3DXVECTOR3				g_vecTMParent = {0, 0, 0}; // 부모의 위치를 나타내는 벡터
...
void InitAnimation()
{
	...
	FLOAT Yaw = 0;// D3DX_PI * 90.0f / 180.0f; // Y축 90도 회전
	FLOAT Pitch = 0;
	FLOAT Roll = 0;
...
void setupMatrix()
{
	D3DXQUATERNION quat;
	//사원수를 회전 행렬값으로 변환
	D3DXQuaternionRotationYawPitchRoll(&quat, g_fRotX, g_fRotY, 0); 
	D3DXMatrixRotationQuaternion(&g_matRParent, &quat);
	
	D3DXMatrixTranslation(&g_matTMParent, g_vecTMParent.x, g_vecTMParent.y, g_vecTMParent.z);
...
LRESULT WINAPI MsgProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_KEYDOWN:	
		...
		case 'a':
		case 'A':
			g_vecTMParent.x -= TRANSLATION_DELTA;
			break;
		case 'd':
		case 'D':
			g_vecTMParent.x += TRANSLATION_DELTA;
		...
		case 'w':
		case 'W':
			g_vecTMParent.y += TRANSLATION_DELTA;
			break;
		case 's':
		case 'S':
			g_vecTMParent.y -= TRANSLATION_DELTA;
