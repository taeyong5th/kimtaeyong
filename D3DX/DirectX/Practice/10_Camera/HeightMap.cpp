...
void ProcessKey(void)
{
	...
	if (GetAsyncKeyState(VK_LEFT)) g_pCamera->MoveLocalX(0.5f);
	if (GetAsyncKeyState(VK_RIGHT)) g_pCamera->MoveLocalX(-0.5f);
	...
	static float distValue = 0.0f;
	static const float distGap = 0.02f;
...
LRESULT WINAPI MsgProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	{
	case WM_CREATE:
		MessageBoxA(hWnd, TEXT("이동 : ↑, ↓, ←, →, a, z\n회전 : 마우스 이동, LShift"), TEXT("조작키"), MB_OK);
		break;
	case WM_DESTROY:
		Cleanup();
		PostQuitMessage(0);
